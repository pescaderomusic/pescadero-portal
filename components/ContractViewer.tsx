...
export default function ContractViewer({ contract }: Props) {
  ...
  async function saveProgress() {
    ...
    try {
      const res = await fetch('/api/client/save-contract', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contractId: contract.id, fields: editableFields }),
      })
      if (res.ok) {
        setSaveStatus('saved')
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #0F1F35 0%, #0A1828 100%)' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <img src={LOGO_SRC} alt="Pescadero Music" style={{ width: 40, height: 40, objectFit: 'contain', filter: 'drop-shadow(0 0 8px rgba(214,48,49,0.4))' }} />
          <span style={{ fontFamily: 'monospace', fontSize: 11, fontWeight: 700, letterSpacing: '3px', color: '#4FB9AF', textTransform: 'uppercase' }}>PESCADERO MUSIC</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={async () => { await saveProgress(); router.push('/dashboard') }} style={{
            background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)',
            borderRadius: 8, padding: '7px 16px', fontSize: 12, fontWeight: 600,
            cursor: 'pointer', color: 'rgba(232,224,213,0.6)', fontFamily: 'Poppins, sans-serif',
          }}>← Save & Exit</button>
          <button onClick={saveProgress} disabled={saveStatus === 'saving'} style={{
            background: saveStatus === 'saved' ? 'rgba(79,185,175,0.2)' : saveStatus === 'error' ? 'rgba(214,48,49,0.15)' : 'rgba(79,185,175,0.1)',
            border: `1px solid ${saveStatus === 'saved' ? 'rgba(79,185,175,0.5)' : saveStatus === 'error' ? 'rgba(214,48,49,0.4)' : 'rgba(79,185,175,0.25)'}`,
            borderRadius: 8, padding: '7px 16px', fontSize: 12, fontWeight: 600,
            cursor: saveStatus === 'saving' ? 'wait' : 'pointer',
            color: saveStatus === 'error' ? '#ff8585' : '#4FB9AF',
            fontFamily: 'Poppins, sans-serif', transition: 'all 0.2s', minWidth: 120,
          }}>
      ...
      {signed ? (
        <div style={{
          ...
          <p style={{ fontSize: 12, color: 'rgba(232,224,213,0.4)', fontFamily: 'Poppins, sans-serif', marginTop: 8 }}>
            Head back to your dashboard to complete your deposit and lock in your date.
          </p>
          <Link href="/dashboard" style={{
            display: 'inline-block', marginTop: 16,
            background: 'rgba(79,185,175,0.1)', color: '#4FB9AF',
            border: '1px solid rgba(79,185,175,0.3)',
      ...
      {showConfirmation && (
        <div style={{
          ...
          <div style={{
            background: '#0F1F35', border: '1px solid rgba(79,185,175,0.25)',
            borderRadius: 16, padding: '40px 36px', maxWidth: 440, width: '100%',
            textAlign: 'center', position: 'relative',
            boxShadow: '0 24px 80px rgba(0,0,0,0.5)',
          }}>
            <button
              onClick={() => { setShowConfirmation(false); router.push('/dashboard') }}
              style={{
                position: 'absolute', top: 16, right: 16,
                background: 'rgba(255,255,255,0.08)', border: 'none',
                borderRadius: '50%', width: 30, height: 30,
                color: 'rgba(232,224,213,0.5)', fontSize: 16,
            ...
            <h2 style={{
              ...
              fontSize: 24, color: 'white', marginBottom: 12,
            }}>Contract Submitted!</h2>
            <p style={{
              fontSize: 13, color: 'rgba(232,224,213,0.6)',
              fontFamily: 'Poppins, sans-serif', lineHeight: 1.7, marginBottom: 8,
            }}>
              Your service agreement has been signed and submitted to Garrett.
            </p>
            <p style={{
              fontSize: 13, color: 'rgba(232,224,213,0.6)',
              fontFamily: 'Poppins, sans-serif', lineHeight: 1.7, marginBottom: 28,
            }}>
              Your next step is the <strong style={{ color: 'white' }}>$100 deposit</strong> to officially lock in your date. Head to your dashboard to complete payment.
            </p>
            <button
              onClick={() => { setShowConfirmation(false); router.push('/dashboard') }}
              style={{
