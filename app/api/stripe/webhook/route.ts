...
import { sendDepositConfirmation, sendFinalPaymentConfirmation, sendGarrettContractSigned } from '@/lib/email'
...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (!contract_id || !client_id) return NextResponse.json({ error: 'Missing metadata' }, { status: 400 })
    ...
    if (!contract || !profile) return NextResponse.json({ error: 'Not found' }, { status: 404 })
    ...
    if (payment_type === 'deposit') {
      ...
      await admin.from('bookings').update({ current_step: 4, step_3_status: 'complete', step_4_status: 'active' }).eq('client_id', client_id)
      ...
      await admin.from('bookings').update({ current_step: 6, step_5_status: 'complete', step_6_status: 'active' }).eq('client_id', client_id)
