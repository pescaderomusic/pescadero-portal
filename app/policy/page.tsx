...
export default function PolicyPage() {
  return (
    <div style={{ minHeight: '100vh', background: `linear-gradient(160deg, ${NAVY} 0%, #0A1828 100%)`, fontFamily: 'Poppins, sans-serif' }}>
      ...
      <nav className="no-print" style={{ position: 'sticky', top: 0, zIndex: 100, background: 'rgba(10,24,40,0.96)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(68,190,199,0.15)', padding: '0 28px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 18, fontWeight: 700, color: RED }}>Pescadero</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <button onClick={() => window.print()} style={{ background: 'none', border: `1px solid rgba(68,190,199,0.3)`, borderRadius: 6, color: BLUE, fontSize: 11, fontWeight: 700, padding: '5px 14px', cursor: 'pointer', fontFamily: 'Poppins, sans-serif' }}>🖨 Print / Save PDF</button>
          <Link href="/dashboard" style={{ fontSize: 11, color: 'rgba(232,224,213,0.35)', textDecoration: 'none', letterSpacing: '1px', textTransform: 'uppercase' }}>← Dashboard</Link>
        </div>
      </nav>
      ...
      <div style={{ borderBottom: `1px solid rgba(68,190,199,0.12)`, padding: '52px 24px 44px', textAlign: 'center' }}>
        ...
        <h1 style={{ margin: '0 0 10px', fontFamily: 'Georgia, serif', fontSize: 38, fontWeight: 700, color: 'white', lineHeight: 1.1 }}>Service Policy</h1>
        <p style={{ margin: '0 auto', fontSize: 14, color: 'rgba(232,224,213,0.45)', fontStyle: 'italic', maxWidth: 480 }}>Professional Sound for Your Perfect Day</p>
        <p style={{ marginTop: 16, fontSize: 12, color: 'rgba(232,224,213,0.25)' }}>garrett@pescaderomusic.com · (210) 727-9328 · pescaderomusic.com</p>
      </div>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '52px 24px 80px' }}>
        ...
        <Section eyebrow="Package" title="What's Included in Every Celebration">
          {[
            ...
            <div key={label} style={{ marginBottom: 12 }}>
              <p style={{ margin: '0 0 2px', fontWeight: 700, color: 'white', fontSize: 13 }}>{label}</p>
              <p style={{ margin: 0, fontSize: 13, color: 'rgba(232,224,213,0.65)' }}>{desc}</p>
            </div>
          ))}
        </Section>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', margin: '16px 0' }}>
            <thead>
              <tr style={{ background: NAVY }}>
                <th style={{ padding: '10px 16px', textAlign: 'left', fontSize: 11, fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)' }}>Day of the Week</th>
            ...
            <tbody>
              {[
                ...
                <tr key={day} style={{ background: i % 2 === 0 ? 'rgba(255,255,255,0.03)' : 'rgba(255,255,255,0.06)' }}>
                  <td style={{ padding: '12px 16px', fontSize: 13, color: 'rgba(232,224,213,0.8)' }}>{day}</td>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 16 }}>
            {[
              ...
              <div key={range} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: 18, textAlign: 'center' }}>
                <p style={{ margin: '0 0 6px', fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>{range}</p>
                <p style={{ margin: '0 0 4px', fontSize: 22, fontWeight: 700, color: BLUE }}>{fee}</p>
                <p style={{ margin: 0, fontSize: 11, color: 'rgba(255,255,255,0.3)', fontStyle: 'italic' }}>{note}</p>
              </div>
            ))}
          </div>
        </Section>
              ...
              <tbody>
                {[['14 days – 72 hours before event','100%'],['72 – 24 hours before event','80%'],['Less than 24 hours','50%']].map(([w,r],i) => (
                  <tr key={w} style={{ background: i%2===0?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.06)' }}>
                    <td style={{ padding: '9px 12px', fontSize: 12, color: 'rgba(232,224,213,0.75)' }}>{w}</td>
        ...
        <div style={{ borderTop: '1px solid rgba(68,190,199,0.12)', paddingTop: 32, textAlign: 'center', marginTop: 16 }}>
          <p style={{ margin: '0 0 6px', fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 18, color: 'white' }}>Thank you for choosing Pescadero Music.</p>
          <p style={{ margin: '0 0 20px', fontSize: 13, color: 'rgba(232,224,213,0.35)' }}>We look forward to making your event a success.</p>
...
function Section({ eyebrow, title, children }: { eyebrow: string; title: string; children: React.ReactNode }) {
  const BLUE = '#44BEC7'; const RED = '#D62828'
  return (
    <div style={{ marginBottom: 44 }}>
      ...
      <h2 style={{ margin: '0 0 16px', fontFamily: 'Georgia, serif', fontSize: 26, color: 'white', fontWeight: 700, borderBottom: `2px solid ${RED}`, paddingBottom: 8 }}>{title}</h2>
      <div style={{ fontSize: 13, color: 'rgba(232,224,213,0.7)', lineHeight: 1.8, fontFamily: 'Poppins, sans-serif' }}>{children}</div>
    </div>
  )
}
...
function PolicyRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <p style={{ margin: '0 0 4px', fontWeight: 700, color: 'white', fontSize: 13 }}>{label}</p>
      <div style={{ fontSize: 13, color: 'rgba(232,224,213,0.65)', lineHeight: 1.7 }}>{children}</div>
    </div>
  )
}
